...
import {VaultType, VaultState, RoleType, ClaimParams, DepositParams, WithdrawParams, OperationData, UserClaimedInfo} from "../../contracts/lib/types/VaultStruct.sol";
...
contract ProtocolVaultFuzzTest is Base {
    ...
    function testFuzz_LpDeposit(uint256 amount) public {
        ...
        assertEq(mockToken.balanceOf(address(protocolVault)), vaultBalanceBefore + amount, "Vault balance did not increase correctly");
...
function testFuzz_MultiLpConcurrentDeposits(uint256[5] memory amounts) public {
    ...
    for (uint i = 0; i < 5; i++) {
        ...
        assertEq(mockToken.balanceOf(lpUsers[i]), userBalanceBefore - amounts[i], "User balance not updated correctly");
    ...
    assertEq(mockToken.balanceOf(address(protocolVault)), initialVaultBalance + totalDeposited, "Vault balance didn't increase by total deposit amount");
